#ejercicios clase 4: Flow Control
...
for city in cities:
    city = city.title()
    tit_cities.append(city.title())
print(cities)                #No change
print(tit_cities)

# modifiying it (need index)
for i in range(len(cities)):
    cities[i] = cities[i].title()
print('')
print(cities)

# 2.- Loop thorugh lists II
...
## If the number is NOT a prime number, it should print "[number] is not a prime number", and a
## factor of that number, other than 1 and the number itself: "[factor] is a factor of [number]"
for i in check_prime:
    divisor = 2
    for divisor in range(2,i//2 + 1):
        if i % divisor == 0:
            print("{} is not a prime number. {} is a factor of {}".format(i,divisor,i))
            break
        if divisor == i//2:
            print("{} is a prime number".format(i))

# 4.- Work with list (Oscar´s nominees and winers)
...
for year, directors in nominated.items():
    for director in directors:
        if director in nominations:
            nominations[director] += 1
        else:
            nominations[director] = 1
...
### Please provide a list with the name(s) of the director(s) with
